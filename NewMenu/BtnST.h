...
// Uncomment the following 2 lines to enable support for CNewMenu class
#define	BTNST_USE_CNEWMENU
#ifdef BTNST_USE_CNEWMENU
	#include "NewMenu.h"
#endif BTNST_USE_CNEWMENU
...
class GUILIBDLLEXPORT CButtonST : public CButton
{
public:
    CButtonST();
	~CButtonST();
...
public:
	void SetTextAlign(const UINT & al){m_TextAlign=al;}
	UINT GetTextAlign(){return m_TextAlign;}
	void SetWordBreak(const BOOL & wb=true){m_bUseWordBreak=wb;}
	BOOL GetWordBreak(){return m_bUseWordBreak;}
	...
	UINT GetMenuDrawMode();
	UINT SetMenuDrawMode(UINT mode);
...
#ifdef	BTNST_USE_CNEWMENU
	DWORD SetMenu(UINT nMenu, HWND hParentWnd, UINT bWinXPStyle = CNewMenu::STYLE_XP, UINT nToolbarID = NULL, CSize sizeToolbarIcon = CSize(16, 16), COLORREF crToolbarBk = RGB(255, 0, 255), BOOL bRepaint = TRUE);
	CNewMenu* GetMenu(){return &m_menuPopup;}
#else
...
#ifdef	BTNST_USE_CNEWMENU
	CNewMenu		m_menuPopup;		// CNewMenu class instance
#else
